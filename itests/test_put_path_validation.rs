...
async fn test_put_path_validation() {
    ...
    let form_data = reqwest::multipart::Form::new()
        .text("test.txt", "test content");
    
    let response = client
        .put(&format!("{}/api/repos/nonexistent/repo/file/main/uploads", server.base_url()))
        .multipart(form_data)
        .send()
        .await
    ...
    assert!(status.is_client_error() || status.is_server_error(), 
        "PUT to non-existent repo should fail - status: {}", status);
    
    // Should get a reasonable error message about the repository not existing
    assert!(body.contains("not found") || body.contains("repository") || body.contains("Repository"), 
        "Should get repository not found error - body: {}", body);
    ...
    let test_cases = vec![
        ...
        ("/api/repos/test/repo/file/main/uploads/subfolder", "Nested directory"),
        ("/api/repos/test/repo/file/feature-branch/uploads", "Feature branch"),
    ];
    ...
    for (path, description) in test_cases {
        ...
        assert!(status.is_client_error() || status.is_server_error(), 
            "PUT should fail appropriately - status: {}", status);
    }
    ...
    let form_data = reqwest::multipart::Form::new()
        ...
        .part("custom", reqwest::multipart::Part::text("Custom content")
            .file_name("custom_name.txt")      // Explicit filename
            .mime_str("text/plain").unwrap());
    
    let response = client
        .put(&format!("{}/api/repos/test/repo/file/main/batch", server.base_url()))
        .multipart(form_data)
        .send()
        .await
        .expect("Failed to send PUT request");
