use crate::constants::{LEFT_COMPARE_COMMIT, RIGHT_COMPARE_COMMIT};
...
fn compute_row_comparison(
    ...
    if let Some(compare_id) = compare_id {
        ...
        maybe_write_dupes(repo, compare_id, &compare_results.dupes)?;
    }
...
mod tests {
    ...
    fn test_compare_cache_miss_when_branch_ref_updates() -> Result<(), OxenError> {
        test::run_compare_data_repo_test_fully_commited(|repo| {
            ...
            let compare = api::local::compare::join_compare::get_cached_compare(
                &repo,
                "a_compare_id",
                &left_entry,
                &right_entry,
            ...
            let maybe_compare = api::local::compare::join_compare::get_cached_compare(
                &repo,
                "no_id",
                &new_left_entry,
                &new_right_entry,
