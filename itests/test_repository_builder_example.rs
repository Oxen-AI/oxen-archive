...
async fn test_repository_builder_advanced() {
    ...
    test_repo.add_file("src/lib.rs", "pub fn hello() { println!(\"Hello from lib!\"); }")
        .expect("Failed to add lib.rs");
    
    test_repo.add_file("tests/integration.rs", "#[test]\nfn test_hello() { assert_eq!(2 + 2, 4); }")
        .expect("Failed to add integration test");

    // Commit the new files
    test_repo.commit("Add library and tests")
