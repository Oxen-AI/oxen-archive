use crate::common::{TestServer, TestRepositoryBuilder};
...
async fn oxen_repo_held_csv_should_be_accessible_via_http_get() {
    ...
    let timestamp = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let test_dir = std::env::temp_dir().join(format!("oxen_csv_test_{:?}_{}", unique_id, timestamp));
    ...
    let response = client
        .get(&format!("{}/api/repos/test_user/csv_repo/file/main/products.csv", server.base_url()))
        .send()
        .await
    ...
    if status.is_success() {
        assert!(body.contains("Laptop") || body.contains("Electronics"), 
            "CSV file content not found - body: {}", body);
