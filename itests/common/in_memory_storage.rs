    ...
    pub fn clear(&self) -> Result<(), OxenError> {
        {
            let mut storage = self.storage
                .lock()
                .map_err(|e| OxenError::basic_str(format!("Failed to acquire storage lock: {}", e)))?;
            storage.clear();
        }
        {
            let mut chunks = self.chunks
                .lock()
                .map_err(|e| OxenError::basic_str(format!("Failed to acquire chunks lock: {}", e)))?;
            chunks.clear();
        }
        Ok(())
    }
...
impl VersionStore for InMemoryVersionStore {
    ...
    async fn store_version_chunk(&self, hash: &str, chunk_number: u32, data: &[u8]) -> Result<(), OxenError> {
        let mut chunks = self.chunks
            .lock()
            .map_err(|e| OxenError::basic_str(format!("Failed to acquire lock: {}", e)))?;
