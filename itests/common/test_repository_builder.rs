...
impl TestRepositoryBuilder {
    ...
    pub async fn build(self) -> Result<TestRepository, OxenError> {
        ...
        if !self.files.is_empty() {
            ...
            let commit_msg = self.commit_message.unwrap_or_else(|| "Initial commit".to_string());
            
            repositories::commits::commit_writer::commit_with_user(&repo, &commit_msg, &user).map(|_| ())?;
        }
...
mod tests {
    ...
    async fn test_basic_repository_creation() {
        ...
        assert_eq!(test_repo.file_content("data.csv").unwrap(), "id,name\n1,Alice\n2,Bob");
        assert_eq!(test_repo.file_content("config.json").unwrap(), r#"{"version": "1.0"}"#);
    }
    ...
    async fn test_add_file_after_creation() {
        ...
        // Add another file
        test_repo.add_file("added.txt", "Added content").await.unwrap();
        assert_eq!(test_repo.files().len(), 2);
