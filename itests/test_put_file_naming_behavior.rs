...
async fn test_put_with_form_field_name_as_filename() {
    ...
    let form_data = reqwest::multipart::Form::new()
        .text("my_data_file.txt", "Content from field name");
    
    let response = client
        .put(&format!("{}/api/repos/test_user/test_repo/file/main/uploads", server.base_url()))
        .multipart(form_data)
        .send()
        .await
...
async fn test_put_multiple_files() {
    ...
    let response = client
        .put(&format!("{}/api/repos/test_user/test_repo/file/main/batch", server.base_url()))
        .multipart(form_data)
        .send()
        .await
        .expect("Failed to send PUT request");
...
async fn test_put_to_root_directory() {
    ...
    let form_data = reqwest::multipart::Form::new()
        .text("root_file.txt", "File in root");
    
    let response = client
        .put(&format!("{}/api/repos/test_user/test_repo/file/main/", server.base_url()))
        .multipart(form_data)
        .send()
        .await
        .expect("Failed to send PUT request");
